...
- `pnpm test` from the repository root currently completes without running package tests because no workspace `test` tasks are wired in Turbo.
...
- The only repository workflow in `.github/workflows/update.yaml` is a manual Medusa update workflow that provides storefront env vars through GitHub secrets.
